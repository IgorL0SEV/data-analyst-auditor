...
- **CSV и Excel**: анализ файлов с таблицами.
- **SQL-базы**: read-only запросы через SQLAlchemy (SELECT, схема).
...
# CSV
python scripts/analyze_csv.py data/file.csv \
  --title "Название" \
  --out-dir temp/YOUR_PROJECT

# Excel
python scripts/analyze_excel.py data/file.xlsx \
  --sheet "Sheet1" \
  --title "Название" \
  --out-dir temp/YOUR_PROJECT

# SQL
python scripts/schema_info.py --db-url "$DATABASE_URL"
python scripts/query_db.py "SELECT * FROM orders LIMIT 10" --db-url "$DATABASE_URL"
```
...
python scripts/analyze_csv.py data/file.csv \
  --title "Sales 2026" \
  --out-dir temp/csv_sales

# Excel
python scripts/analyze_excel.py data/file.xlsx \
  --sheet "Sheet1" \
  --title "Sales 2026" \
  --out-dir temp/excel_sales

# SQL
python scripts/schema_info.py
python scripts/query_db.py "SELECT * FROM orders LIMIT 10"
```
...
data-analyst-auditor/
├── scripts/
│   ├── analyze_nocodb.py   # основной анализатор NocoDB
│   ├── analyze_csv.py      # анализ CSV
│   ├── analyze_excel.py    # анализ Excel
│   ├── query_db.py         # read-only SQL-запросы
│   ├── schema_info.py      # схема SQL-базы
│   ├── table_analyzer.py   # общий движок: summary, charts, report
